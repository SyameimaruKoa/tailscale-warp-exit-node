...
setup_routes() {
    ...
    echo "Background: Setting up IPv4 NAT..."
    iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE
    
    echo "Background: Setting up IPv6 NAT..."
    # 失敗しても警告を出して進む
    if ip6tables -t nat -A POSTROUTING -o eth0 -j MASQUERADE; then
        echo "Background: IPv6 NAT configured."
    else
        echo "WARNING: IPv6 NAT failed. Did you run 'sudo modprobe ip6table_nat' on the host?"
    fi
